...
class OnyomiApp:
    ...
    def mostrar_historial(self):
        history = self.progress_service.get_user_progress_history("onyomi", self.username)
    ...
    def mostrar_nuevo_kanji(self):
        ...
        self.mostrar_opciones_onyomi()


    def mostrar_opciones_onyomi(self):
        self.opciones = set()

        self.opciones.add(self.kanji_actual.onyomi)

        while len(self.opciones) < 4:
            try:
                nuevo_kanji = self.kanji_service.get_random_kanji()
                if nuevo_kanji.onyomi != self.kanji_actual.onyomi:
                    self.opciones.add(nuevo_kanji.onyomi)
            except ValueError:
                break
    ...
    def verificar_respuesta(self, opcion):
        if self.opciones[opcion] == self.kanji_actual.onyomi:
            messagebox.showinfo("Correcto", "¡Respuesta correcta!")
            self.progress_service.update_progress("onyomi", self.username, self.kanji_actual.kanji, is_correct=True)
        else:
            messagebox.showerror("Incorrecto", f"Incorrecto. La respuesta correcta es: {self.kanji_actual.onyomi}")
            self.progress_service.update_progress("onyomi", self.username, self.kanji_actual.kanji, is_correct=False)
